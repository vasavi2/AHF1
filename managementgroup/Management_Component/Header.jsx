...
export const Header = ({assetId,sensorId,rowsPerPage,offset,sample,headerpass}) => {
...
const loadList = async () => {
  ...
  if(assetId>0 && sensorId>0){
    console.log("assetid2------>",assetId)
    console.log("sensorid2----->",sensorId)
    // const GA_URL = process.env.REST_URL + "/getAlertListAlert/";
    const GA_URL="http://127.0.0.1:9090/getAlertListAlert/"
    const result = await axios.get(
            `${GA_URL}${assetId}/${sensorId}`
  ...
  const  getLatestRowDetails = async () => {
    if(headerpass){
    const results = await axios.get("http://127.0.0.1:9090/api/getLatestOpenedRow");
    console.log("results2<--->",results)
    setRows(results.data);
    console.log("getrows:-")
    // getData(getRows)
    console.log("getrows:-",getRows)
    console.log("getrows2:-",results.data)
  };
...
const updateNotification=()=>{
  axios.post('http://127.0.0.1:9090/api/updateNotificationAlerts')
  .then((result)=>{
                    ...
                    <Grid xs={12}>
                       <TableContainer component={Paper} style={{maxHeight:'400px',overflow:'auto'}}>
                        <Table sx={{ minWidth: 250 }} size="small" aria-label="a dense table">
                            <TableHead>
                                <TableRow>
                                <TableCell component="th" ><b>Asset</b></TableCell>
                                    <TableCell component="th" ><b>Alerts</b></TableCell>
                                    <TableCell component="th"><b>Start Time</b></TableCell>
                                    <TableCell component="th"><b>End Time</b></TableCell>
                            ...
                            {loadData.map((row) => (
                               ...
                               <TableRow style={{ backgroundColor: row.alert_status === "opened" ? "green" : "white"}}>
                                    <TableCell> {row.asset_name} </TableCell>
                                    <TableCell> {row.alert.charAt(0).toUpperCase()+row.alert.slice(1)} </TableCell>
...
{/*  */}
  ...
  {/*  */}    <div className="flex-header-container">
    <Box sx={{ flexGrow: 1 }} >
      <AppBar position="static" style={{marginLeft:"5px"}}>
        ...
        <HomeIcon style={{fontSize:"40px"}}/>
          ...
          <Box sx={{ flexGrow: 1 }} />
          <Box sx={{ display: { xs: 'none', md: 'flex' } }} style={{marginRight:"50px"}}>
            ...
            <IconButton
              size="large"
              aria-label="show 17 new notifications"
              color="inherit" style={{marginRight:"35px"}} 
            >
              {/* <Badge badgeContent={17} color="error"  > */}
              <Badge badgeContent={getRows === '0' ? "0" : getRows }  color="error"  >

                <NotificationsIcon onClick={handleOpens}></NotificationsIcon> 
            ...
            <IconButton
              ...
              color="inherit"
            >
              <p style={{color:"black",fontSize:"15px",marginRight:"5px"}}>Shaikshavali</p>
              <AccountCircle />
            </IconButton>
    ...
    <BootstrapDialog
      ...
      >
        ...
        <DialogContent dividers>
          ...
          App Studio Playground is an inhouse tool available to play around with the reusable components available in App Studio. To check its input, output and to check whether it fits the project in hand. Its access is limited to LTTSites.
          <p className='p-text'>There are two technologies available at present- React and Angular.</p>
          ...
          {/* <img src={img3} alt="chat Info"/> */}
           ...
           <h6>App Studio Playground Components:</h6>
          <Typography className='p-font' gutterBottom>
          Currently in our App Studio playground there are total 11 categories are there in react as well as angular.
          ...
          <p className='p-text'>1. By using the App Studio library (Clicking on 'library' icon for the respective component on the playground )</p>
          ...
          <h6>App Studio Community</h6>
          <h6>Introduction:</h6>
          <Typography  className='p-font' gutterBottom>         
          App Studio Community is a web-based application for the registration of people who want to either consume or contribute to the App Studio Playground. The Contributor request must be approved by the owners before the user is allowed to join in Microsoft teams and Azure DevOps groups. By following this procedure, the contributor can create new react and angular components.
          </Typography>
          <h6>App Studio Community Workflow:</h6>
          <p className='p-text'>
          The App Studio Community is developed in Power apps. The user must first login using his outlook credentials. Upon successful login, the user will be taken to the App studio community Dashboard and Microsoft Teams, where they can request to be added to the App studio consumers group. After clicking on join, the admins of the App studio consumers group must approve the request before the user can join that group.
          </p>
          <p className='p-text'>
          In the App studio community Dashboard, there is an option called “Be a contributor” on clicking that option the users who want to contribute for our App studio they can get from here. Once the user has selected the "Be a Contributor" option, they must add their primary and secondary abilities.
          </p>
          <p className='p-text'>
          ...
          <h6>Usage of React / Angular components</h6>
          <Typography className='p-font' gutterBottom>       
          All React and Angular components are deployed in Azure Artifacts, users can download components from Azure Artifacts. If you're not aware of how to download, please refer this below document.
          <p className='p-text'>
            <a href='https://webappstudio-react.visualstudio.com/WebAppStudio-React/_wiki/wikis/WebAppStudio-React.wiki/58/How-To-Use-App-Studio-Libraries' target="_blank" rel="noreferrer">ReferenceDoc-HowToUseAppStudioLibraries - Overview (visualstudio.com)</a>
          </p>
          </Typography>
